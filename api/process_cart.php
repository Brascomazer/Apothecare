...
if (isset($_POST['remove']) && is_array($_POST['remove'])) {
    foreach ($_POST['remove'] as $medicijn_id) {
        $remove_sql = "DELETE FROM bestelling_medicijn 
                      WHERE bestelling_id = ? AND medicijn_id = ?";
        $remove_stmt = $conn->prepare($remove_sql);
        $remove_stmt->bind_param("ii", $bestelling_id, $medicijn_id);
        $remove_stmt->execute();
        $remove_stmt->close();
    }
}
...
// Verwijder lege bestellingen
if ($count == 0) {
    $delete_sql = "DELETE FROM bestelling WHERE bestelling_id = ?";
    $delete_stmt = $conn->prepare($delete_sql);
    $delete_stmt->bind_param("i", $bestelling_id);
    $delete_stmt->execute();
    $delete_stmt->close();
}

// Doorsturen naar betaling of winkelwagen
if (isset($_POST['checkout']) && $count > 0) {
